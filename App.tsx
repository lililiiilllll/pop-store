...
    // 실제 연동 시: await supabase.auth.signInWithOAuth({ provider: 'toss' });
  };

  // 3. 신규 추가: 네이버 로그인 핸들러
  const handleNaverLogin = async () => {
    console.log("네이버 인증 시작");
    // 실제 연동 시: await supabase.auth.signInWithOAuth({ provider: 'naver' });
  };
...
                  className="w-full py-4 bg-[#3182f6] text-white font-bold rounded-2xl flex items-center justify-center gap-2 active:scale-95 transition-transform"
                >
                  토스로 시작하기
                </button>
...
                  className="w-full py-4 bg-[#03C75A] text-white font-bold rounded-2xl flex items-center justify-center gap-2 active:scale-95 transition-transform"
                >
