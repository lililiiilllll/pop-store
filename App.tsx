...
useEffect(() => {
  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen overflow-hidden bg-white relative">
      ...
      {/* 2. PC 전용 사이드바 (1024px 이상에서만 보임) - hidden lg:flex */}
      ...
      <div className="flex-1 flex flex-col h-full w-full relative min-h-0">
         ...
         {activeTab === 'home' && (
             ...
             >
                {/*
                    [중요] 바텀시트 컨테이너 자체는 pointer-events-none을 주어
                    시트의 빈 영역(또는 헤더 위쪽의 투명 영역 등)이 지도를 가리지 않게 하되,
                    내부 콘텐츠(핸들, 리스트 등)는 pointer-events-auto로 감싸 터치를 활성화함.
                */}
