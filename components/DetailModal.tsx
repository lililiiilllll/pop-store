...
const DetailModal: React.FC<DetailModalProps> = ({ store, onClose, isLiked, onShowSuccess }) => {
  ...
  return (
    <div className="flex flex-col h-[85vh] lg:h-auto max-h-[90vh] bg-white overflow-hidden">
      {/* 상단 이미지 영역 */}
      <div className="relative h-64 lg:h-80 w-full bg-gray-200">
        <img 
          src={store.imageUrl} 
          alt={store.name} 
      ...
      <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
        <div className="flex justify-between items-start mb-4">
          <div>
            <span className="inline-block px-2 py-1 rounded bg-blue-50 text-blue-600 text-xs font-bold mb-2">
        ...
        <div className="space-y-4 text-sm text-gray-600 mb-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <span>{store.description || '상세 설명이 등록되지 않았습니다.'}</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mt-auto">
          ...
          <button className="py-4 bg-gray-100 text-gray-900 rounded-2xl font-bold hover:bg-gray-200 transition-all">
            길찾기
          </button>
        </div>
      </div>
